...
describe("Platzi token tests", function () {
  ...
  describe("V1 tests", function () {
    before(async function () {
      ...
      platziTokenV1 = await upgrades.deployProxy(PlatziToken, [initialSupply], {
        kind: "uups",
      });
    ...
    it("Should run into an error when executing a function that does not exist", async function () {
      expect(() =>
        platziTokenV1.mint(deployer.address, ethers.BigNumber.from(10).pow(18))
      ).to.throw();
    });
  });
  ...
  describe("V2 tests", function () {
    before(async function () {
      ...
      platziTokenV2 = await upgrades.upgradeProxy(
        platziTokenV1.address,
        PlatziTokenV2
      );
    ...
    it("Should mint tokens when the owner is executing the mint function", async function () {
      ...
      const accountAmountBeforeMint = await platziTokenV2.balanceOf(
        deployer.address
      );
